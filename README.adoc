...
+
* send a (gentle!) reminder to the reporter that we need feedback before we can do anything.
...
quite some time ago.
+
There might be feedback there, in which case you might want to remove the `triage/needs-reproducer` label
...
The label identifying GitHub issues for which feedback was requested.
+
String, mandatory, no default.
